...
} from './LinearPlasmidAnnotations';
import {
  calculateAnnotationDimensions,
  calculateTrackY,
  assignTracks,
} from './LinearPlasmidAnnotations/utils';
...
export const LinearPlasmidViewer = forwardRef<LinearPlasmidViewerRef, LinearPlasmidViewerProps>(
  (
    ...
    const TOTAL_LINE_HEIGHT =
      FEATURE_TRACK_HEIGHT * MAX_FEATURE_TRACKS +
      SEQUENCE_HEIGHT +
      POSITION_HEIGHT +
      FEATURE_SECTION_GAP +
      LINE_SPACING +
    ...
    const renderLine = (lineIndex: number) => {
          ...
          if (isBaseSelected(pos, selectedRegion.start, selectedRegion.end)) {
            ...
            selectionHighlights.push(
              <rect
                ...
                width={CHAR_WIDTH}
                height={
                  backboneY + BACKBONE_HEIGHT + SELECTION_PADDING_BOTTOM + SELECTION_PADDING_TOP
                }
                fill="#FFD700"
                opacity="0.3"
                pointerEvents="none"
      ...
      return (
        <g
          key={lineIndex}
          transform={`translate(${MARGIN_LEFT}, ${y})`}
        ...
        >
          ...
          <g transform={`translate(0, ${positionY})`} pointerEvents="none">
            <text x="-5" y={0} textAnchor="end" dominantBaseline="middle" fontSize="10" fill="#666">
