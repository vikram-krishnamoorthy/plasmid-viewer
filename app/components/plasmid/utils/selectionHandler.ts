...
export class CircularSelectionHandler implements SelectionHandler {
  ...
  private findTranslationFeature(position: number): Feature | null {
    return (
      this.features.find(
        (f) => f.type === 'translation' && position >= f.start && position <= f.end
      ) || null
    );
  }

  private snapToCodonBoundaries(
    start: number,
    end: number,
    feature: Feature
  ...
  handleSelectionMove(position: number): SelectedRegion | null {
    ...
    const clockwiseDistance =
      position >= start ? position - start : this.plasmidLength - start + position;

    const counterclockwiseDistance =
      position >= start ? this.plasmidLength - position + start : start - position;
