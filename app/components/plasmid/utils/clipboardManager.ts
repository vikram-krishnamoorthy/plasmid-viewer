...
export class CircularClipboardManager implements ClipboardManager {
  ...
  private findTranslationFeature(start: number, end: number, features: Feature[]): Feature | null {
    ...
    return (
      features.find((f) => f.type === 'translation' && start >= f.start && end <= f.end) || null
    );
  }
  ...
  private getSequenceSegment(
    ...
    if (translationFeature) {
      ...
      const { start: codonStart, end: codonEnd } = this.snapToCodonBoundaries(
        start,
        end,
        translationFeature
      );
