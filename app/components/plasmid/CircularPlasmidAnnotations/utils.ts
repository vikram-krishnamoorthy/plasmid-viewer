...
export const calculateFeatureRadius = (track: number, _maxTracks: number): number => {
...
export const doFeaturesOverlap = (f1: Feature, f2: Feature, plasmidLength: number): boolean => {
...
export const hasTrackPriority = (f1: Feature, f2: Feature, plasmidLength: number): boolean => {
...
export const assignCircularTracks = (
  ...
  while (hasOverlaps && currentTrack < _maxTracks) {
    ...
    const featuresInTrack = features.filter((f) => trackAssignments.get(f.id) === currentTrack);
